...
void matrixMulMatrix(const Matrix2D *matrix1, const Matrix2D *matrix2T, Matrix2D *result) {
    assert(matrix1->cols == matrix2T->cols);
    for (size_t i = 0; i < matrix1->rows; ++i) {
        for (size_t j = 0; j < matrix2T->rows; ++j) {
            float sum = 0.0f;
            for (size_t k = 0; k < matrix1->cols; ++k) {
                sum += (MATRIX2D_AT(*matrix1, i, k)) * (MATRIX2D_AT(*matrix2T, j, k));
            }
